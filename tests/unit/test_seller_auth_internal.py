...
의존성 함수를 직접 호출한다(HTTP 서버 없음). 검증 항목:
  ...
  - internal_api_token 미설정(dev) → 검증 스킵하되 신원 헤더는 여전히 필수 (2026-07-20 팀 규약 단일 키 통일)
"""
...
def test_valid_token_and_identity(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _patch_settings(monkeypatch, internal_api_token=_TOKEN)
...
def test_dev_skip_without_internal_api_token(monkeypatch: pytest.MonkeyPatch) -> None:
    """internal_api_token 미설정(dev) → 토큰 검증 스킵, 신원 헤더만으로 통과."""
    _patch_settings(monkeypatch, internal_api_token="")
