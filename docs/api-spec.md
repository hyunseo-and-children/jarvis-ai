...
- **[v0.15.15, 이슈 #62]** session-end(§3.5) 멱등 키는 별도 `eventId` 필드 대신 **본문 신원에서 파생**한다 — `session-end:{userId}:{sessionId}`. 동일 (userId, sessionId) 재전송은 중복 처리되지 않는다(BE 실측 payload 정합).
...
- **멱등**: 멱등 키 = `session-end:{userId}:{sessionId}` 파생(§2.7). 동일 (userId, sessionId) 재수신도 델타·프로필 중복 처리 안 함(REQ-PROF-051).
