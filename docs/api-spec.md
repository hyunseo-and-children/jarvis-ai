...
> 동기화: **v0.15.0 기준 2026-07-17**.

| 항목 | 값 |
|---|---|
| 문서 버전 | v0.15.0 |
| 작성일 | 2026-07-14 (v0.15.0 개정 2026-07-17) |
| 상태 | draft |
...
**[HARD, 개정 v0.15.0] `productId` = 경계별 타입 분리** — **[07/17 BE 확정]** 상품 원본 id는 **`BIGINT` 숫자**다(ERD v2·I-19 본문 명시). 따라서 **AI↔Spring internal 계약(I-1 검색·I-2 장바구니·I-19 구매 이력·I-21 추천 push)은 숫자 id를 그대로** 쓴다. **SSE/FE-대면 경계**(`products.ready`·`draft` 등)에서는 **문자열로 정규화**하며 AI가 경계에서 변환한다. 구 "전 구간 문자열" 규칙은 **FE-대면 경계 한정**으로 축소된다(결정 9-B의 표시 권위=Spring 취지는 유지).

> **[🟡 우리 정렬 / 확인]** 이 경계 변환 정책(internal 숫자 ↔ SSE 문자열)은 우리 코드(`schemas/spring.py`·SSE 스키마)에 반영이 필요하다. SSE에서 `productId`를 문자열로 유지하는 최종안이 FE와 합의됐는지 확인 대상(§5 C-5).
...
[3] AI: 콜백 성공 → SSE `products.ready`({ sessionId, listId }) emit + reason은 SSE 직접 (§3.1)
...
- **`listId`는 FastAPI 생성** — 구 "Spring이 listId 발급" 가정 폐기. TTL·유효시간은 세션(10분) 이하 제안, 🔴 확정.
- **`reason`(추천 근거)은 SSE로 FE에 직접** — 콜백에 불포함(§3.1). 확정 필요 🔴.
...
- **카드 응답 스키마는 FE ↔ Spring이 확정**(LLM 사안 아님). 추천 `reason`은 SSE로 오므로(§4.2) FE가 카드와 결합하는 방식만 정하면 된다. 스키마 OPEN 🔴(§5 C-12).
...
| Q2 | **I-21** 스키마·`listId` TTL·`reason` SSE 직접 | ✅ 스키마 수용(§4.2). listId=우리 생성, TTL≤세션(10분), reason=SSE 직접 | 즉답 |
| Q3 | **[적용]** = `{action:"confirm", draftId}` 확정? | ✅ **예** — §3.2 HITL 설계와 동일 | 즉답 |
| Q4 | **I-17** 커서(수정시각+id)·`attributes` JSON·리뷰 텍스트 | 커서 수용, attributes 스키마 협의, 리뷰 텍스트 MVP 제외 권장(§4.8) | 부분 결정 |
| Q5 | **I-13** 본문 재작성(현재 I-5 내용 복붙) | BE 재작성 대기 — 우리는 실제 행동이벤트 조회 스키마만 필요(§4.4 I-13) | BE 몫 |
| Q6 | **CH-3**(CS 챗) FE-AI 직접 연결? | 결정 필요 — CH-2와 통일(FE-AI 직접) 권장 | **팀 결정** |
...
> **팀 결정 필요는 Q6(CS챗 라우팅)뿐**이고 Q4는 부분. 나머지 7건은 기존 계약과 일치해 "예" 회신 가능. Q5(I-13)는 BE 재작성 대기.
