...
> 동기화: **v0.15.9 기준 2026-07-19**.

| 항목 | 값 |
|---|---|
| 문서 버전 | v0.15.9 |
| 작성일 | 2026-07-14 (v0.15.9 개정 2026-07-19) |
| 상태 | draft |
...
- **⚠️ [dedup 갭 — 🔴 BE 요청 v0.15.9] items에 `category` 필드 없음** — 결정 14-F의 소모품 **카테고리 억제**·되돌리기 `suggestions.revert.category` 칩(§3.1)은 category가 **유일 소스**인데 I-19가 주지 않아 **구현 불가**(exact `productId` 제외는 구현 완료). **[LLM팀 요청] I-19 items[]에 `categoryName`(string) 추가** — I-1(§4.6)이 이미 `categoryName`을 반환하므로 동일 필드·join, BE 부담 최소. 예: `{ ..., "categoryName": "무선이어폰" }`. 확정 시 소모품 억제·revert 칩 구현. BE 확정 대기 🔴(C-6).
...
| C-6 | **[정정 v0.15.5] 구매 이력 = I-19** | `GET /internal/members/{id}/orders`(§4.7). camelCase·숫자 id(DDL)·`shippingFee` 0. **`status` = 6종**(`PAID/PREPARING/SHIPPING/DELIVERED/CANCELED/RETURNED`, Notion I-19) — 구 representativeStatus 8종은 **O-3(FE `/api/orders`) 오용** 폐기. **`category` 없음**(카테고리 dedup 불가, productId dedup만) | I-19 / Notion·DDL | 🟢 확정(status·타입). 🔴 **[요청 v0.15.9] I-19 items[]에 `categoryName` 추가**(소모품 카테고리 억제·revert 칩 유일 소스, I-1과 동일 필드라 BE 부담 최소) · Notion 페이지 stale(snake/문자열 id) BE 통보 |
