...
| (c) 역방향(질의 시점) | **AI → Spring** | 후보 검색(I-1, §4.6), 구매 이력 조회(§4.7), 장바구니 담기(I-2, §4.1)·조회(I-18, §4.9), 추천 목록 push(§4.2), 판매자 집계(I-6/7/13/14/15/16, §4.4), 상품 CRUD(I-9/10/11/12, §4.5), 생성물 배치(I-17, §4.8) | **전부 서비스 토큰(internal, `X-Internal-Token`) + 본문/쿼리 신원**(AI가 JWT `sub`에서 도출) — BE 실측 정합(v0.13.0) | 결정 7 / 경로 B / BE DB 정합 |
...
```
GET {SPRING_BASE_URL}/internal/orders/recent?window={days}&userId={id}   ← 경로 제안(BE DB 미등재 — 신규 요청)
...
| C-6 | **구매 이력 조회 API — BE DB 미등재(신규 요청)** | 질의 시점 조회(§4.7), **서비스 토큰 + userId(AI 도출)**, 응답 3필드(`productId`·`category`·`purchasedAt`). dedup(14-F)·프로필 소스 공용, 게스트 스킵. BE DB에 전용 엔드포인트 없음(I-4는 주문 상태 요약) → **AI팀 신규 요청 필요** | 결정 14-F/16 | 🔴 **BE에 신설 요청** — 경로·window |
