...
> 동기화: **v0.15.7 기준 2026-07-19**.

| 항목 | 값 |
|---|---|
| 문서 버전 | v0.15.7 |
| 작성일 | 2026-07-14 (v0.15.7 개정 2026-07-19) |
| 상태 | draft |
...
| C-3 | **[v0.6.0 재작성] 장바구니 담기 API(I-2)** | **BE 문서 채택**: `POST /internal/cart/items` 단건 + `X-Internal-Token` + 본문 신원(JWT `sub` 유래) + `optionId` + quantity 1~99 합산 + **게스트 담기 허용**. 옵션 되물음 멀티턴(`CART_OPTION_REQUIRED` options 목록)(§4.1) | BE I-2 문서 / 결정 7 / 결정 8 개정(§8 항목 7) | 🟢 **[해소 v0.15.5] 담기 시 재고검증 없음** — DDL상 재고 차감은 주문 시점, SOLD_OUT 상태 미도입(품절=stock 0). **`OUT_OF_STOCK` 담기 오류 폐기**(BE C-2 페이지 오류코드에도 없음). 🔴 잔여 — options 목록 스키마·서비스 토큰 발급 |
