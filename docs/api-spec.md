...
- **멱등**: 토큰·요청 스키마 검증 뒤, 버퍼 조회보다 먼저 **`session-end:{userId}:{sessionId}` 고정키**를 원자적으로 판정한다(§2.7). 신규 통지는 버퍼가 없어도 `accepted`로 기록하며, 같은 통지 재수신(at-least-once)은 항상 `duplicate`로 답하고 델타·프로필을 중복 처리하지 않는다(REQ-PROF-051). 내부 처리 실패는 버퍼를 보존하고 마킹을 되돌려 재시도를 허용한다.
...
| 버전 | 날짜 | 변경 |
|---|---|---|
| v0.15.17 | 2026-07-22 | **[이슈 #62/#64] I-20 실측 계약과 멱등 응답 순서 확정.** 요청을 `{sessionId,userId(number BIGINT),reason?}`로 정렬하고 `eventId`·`endedAt`를 제거. `userId`는 양의 정수만 엄격히 허용(string/float/bool coercion 거부). 토큰·스키마 검증 → `(userId,sessionId)` 고정키 멱등 판정 → 버퍼 처리 순서를 확정해, 첫 빈 버퍼 통지는 `202 accepted`, 이후 동일 통지는 버퍼 상태와 무관하게 `202 duplicate`가 된다. 내부 실패는 버퍼 보존·마킹 해제로 재시도 가능. Spring PR #24 송신 계약과 정합. |
