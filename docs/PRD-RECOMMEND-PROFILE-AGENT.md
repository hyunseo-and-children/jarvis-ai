...
| 트리거 | 정합성의 원천은 **저장된 회원 세션 버퍼**. Spring I-20(`logout`·`newConversation`)과 AI 내부 10분 inactivity sweep이 같은 멱등 finalizer로 수렴하며, 탭 닫기 신호는 사용하지 않음 | REQ-PROF-050~059 |
...
Spring I-20: logout/newConversation ───┐
                                      ├──▶ 공통 session finalizer (고정 멱등키)
AI idle scheduler: inactivityTimeout ─┘        │
...
**`POST /events/session-end`** — Spring → AI, best-effort·멱등. `{sessionId, userId(number BIGINT), reason?}`이며 알려진 Spring 사유는 `logout`·`newConversation`이다. 신규/중복 모두 202(`accepted`/`duplicate`), 멱등키=`session-end:{userId}:{sessionId}`. `inactivityTimeout`은 AI 내부 스케줄러가 같은 finalizer로 처리하고 탭 닫기 신호는 두지 않는다. — api-spec §3.5
