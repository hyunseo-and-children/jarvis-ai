...
- 두 경로는 같은 finalizer와 `session-end:{userId}:{sessionId}` claim으로 직렬화한다. Spring I-20만 키를 영구 완료하고 idle은 checkpoint 성공 뒤 claim을 해제한다. 새 회원 발화 저장은 이전 processing/completed 키를 activity 갱신과 같은 transaction에서 무효화하며, 진행 중 terminal finalizer는 새 activity를 completed로 덮지 않는다. AI가 자기 HTTP 엔드포인트를 호출하지 않는다.
