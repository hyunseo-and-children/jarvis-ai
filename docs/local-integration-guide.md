...
- 두 경로는 같은 finalizer와 `session-end:{userId}:{sessionId}` 멱등키를 사용한다. AI가 자기 HTTP 엔드포인트를 호출하지 않는다.
- 탭 닫기 전용 신호/API는 없다. 10분 안에 돌아와 발화하면 활동 시각이 갱신되고, 돌아오지 않으면 inactivity timeout이 처리한다.
