...
BE I-1 검색이 카테고리로 상품을 정제하면 검색 품질이 크게 오른다. 따라서 **성공 흐름은 항상
canonical 카테고리를 최소 1개** 낸다(never-null). 단 **검증이 불가능한 실패 시엔 canonical 이 아닌
null 로 degrade** 한다 — 미검증 raw 를 내보내는 대신 카테고리를 빼고 keyword 로 검색한다(§5·#20):

- decompose는 애매한 질의("집들이 선물")도 best-guess를 내도록 유도(정말 모르면 null 허용 →
  그 leg 의 query 또는 발화 임베딩으로 흡수, #17).
