...
발화 원문
  ...
  │     ├─ raw == null                    → embed(그 leg 의 query, 없으면 발화) → top-1 (앵커 우선순위 raw→query→발화, #17)
  │     └─ 하드 실패(embed/DB 다운)       → raw 그대로(검증 없이) or 해당 leg 없음 (§5)
  │     → canonical 리스트(순서보존 dedup, category_fanout_max 절단). **never-null: 항상 ≥1개.**
  │     매핑 호출 자체가 예외면(버그) 빈 legs 로 degrade → filters.category=None (canonical-or-null, #18).
  │
...
들어간다 — legs 가 비면 None(#13), 매핑 호출 예외도 None(#18). 미검증 raw 는 어느 경로로도 새지
않는다. **단, §5 하드실패(DB 다운)의 raw 폴백은 예외** — 이때 저장된 raw 가 다음 리파인 턴에
승계될 수 있으나(비-canonical), DB 복구 후 새 질의 한 번이면 매핑 재실행으로 자가 치유되는 §5
degrade 범위 내 허용치다.
